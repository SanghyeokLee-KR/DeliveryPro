...
**음식 주문부터 라이더 배달, 매장 운영, 관리자 통계까지 하나로 이은 배달 플랫폼**

인천일보아카데미 5인 팀 프로젝트 · 조장 · 서버 공통 규약과 소셜 로그인, 화면 담당
...
![라이더 묶음배달 경로](docs/images/rider-route.jpg)

라이더 한 명이 인천 미추홀구 일대 다섯 곳을 한 번에 도는 화면.
방문 순서와 구간 경로는 서버가 계산했고, 지도 API 의 길찾기를 쓰지 않았다.
...
**설계**
- [ERD / 주요 테이블 설계](#erd--주요-테이블-설계)
- [시스템 구조](#시스템-구조)
- [성능 및 보안 고려사항](#성능-및-보안-고려사항)
- [프로젝트 구조](#프로젝트-구조)

**그 외**
- [기술 스택](#기술-스택)
- [실행 방법 (Oracle)](#실행-방법-oracle)
- [향후 개선 계획](#향후-개선-계획)
...
카카오 JavaScript 키를 환경변수로 주면 실제 지도 위에 경로가 얹힙니다. 키가 없으면 라이더 배달
화면이 지도 SDK 대신 인라인 SVG 로 내려가, 서버가 계산한 경로와 매장·배달지를 그대로 그립니다.
주소 좌표도 카카오 지오코딩이 막히면 데모용 대체 변환으로 넘어갑니다. 실제 좌표가 아니라 주소
문자열에서 유도한 값이고 h2 프로파일에서만 뜹니다. 카카오 디벨로퍼스에서 JS SDK 도메인에
...
방문 순서를 도로망 그래프 위에서 계산해 `order_sequence` 에 저장합니다.

노드는 도로 교차점, 간선은 교차점 사이의 도로 구간인 유향 그래프입니다. 노드 525개,
간선 1,742개이고, 일방통행은 간선 방향으로, 통과 불가 축은 간선 부재로 표현했습니다.
간선 가중치는 거리가 아니라 도로 위계별 통행 속도에서 유도한 소요시간(초)이라 균일하지
않습니다. 균일했다면 너비 우선 탐색으로 충분했을 것입니다.

계산은 두 단계입니다. 정차지마다 다익스트라를 한 번씩 돌려 단일 출발 다중 도착으로
(N+1)x(N+1) 소요시간 행렬을 만들고, 그 행렬 위에서 방문 순서를 정합니다. 순서 결정은
다익스트라가 아니라 별도의 비트마스크 동적계획법(Held-Karp)이 맡습니다. 다익스트라는 단일
출발 최단경로 알고리즘이지 방문 순서를 정하지 않습니다. 묶음 규모가 12곳 이하라 근사가
아니라 정확해이고, 그보다 크면 최근접 이웃과 2-opt 로 내려가면서 근사임을 결과에 표시합니다.
우선순위 큐는 `java.util.PriorityQueue` 를 쓰고, 키 감소 연산이 없어 중복 삽입 후 확정
노드를 버리는 방식으로 처리합니다. 가중치가 시간과 거리라 항상 양수여서 벨만포드가 필요
없습니다.

주소는 도로 위에 있지 않아 가장 가까운 교차점에 붙인 뒤 탐색합니다. 이 진입 거리를 버리면
80m쯤 떨어진 두 배달지가 같은 교차점에 붙었을 때 구간 비용이 0초로 나옵니다. 그래서 스냅
거리를 도보 속도로 환산해 구간 양끝에 더하고, 화면에 그리는 좌표열에도 진입 구간을 붙입니다.
정차지마다 도로를 벗어났다 돌아오는 왕복이 실제로 생기므로 구간마다 두 번 계산합니다.

그래프가 실제로 작동하는지는 우회 사례로 확인합니다. `(126.692950, 37.453087)` 과
`(126.690714, 37.452494)` 는 직선으로 208m인데 도로로는 1,876m로 9.01배 돌아갑니다.
통과 불가 축 반대편이라 우회로가 하나뿐입니다. 같은 두 지점인데 갈 때 247.7초, 올 때
97.6초로 두 배 넘게 벌어지고, 이것이 일방통행이 비용에 반영됐다는 증거입니다.
좌표와 기댓값은 `DetourCaseTest` 에 상수로 박혀 있습니다. 도로망을 손대면 이 문서가 아니라
테스트가 먼저 깨집니다.

계산 결과는 화면이 그대로 그립니다. 카카오 내비 응답이 있으면 그쪽이 도로 데이터가 촘촘해
먼저 쓰고, 비어 있으면 서버가 내려준 구간 좌표열로 대체합니다.

도로망 데이터의 출처를 분명히 해 둡니다. 실제 지도 데이터를 내려받은 것이 아니라, 서비스
구역 범위 안에 도로 위계와 일방통행, 통과 불가 축을 재현한 합성 도로망입니다. 런타임에
외부 지도 API를 호출하지 않으려고 이렇게 두었습니다. 실제 지도로 바꿀 때는 그래프를 만드는
클래스 하나만 교체하면 되고 탐색과 정렬 계층은 그대로입니다.
`domain/routing/` `domain/order/service/OrderService.java`
...
- 테스트가 41건입니다. 경로 계산과 IP 추출 외에는 사실상 없어서 고칠 때마다 화면을 직접
  눌러 확인했습니다.
- 경로 소요시간은 도로 위계별 상수 속도에서 나온 값이라 신호 대기, 시간대별 정체, 회전
  금지가 들어 있지 않습니다. 카카오 내비 응답과의 실측 대조는 하지 않았습니다.
- 방문 순서의 목적함수가 총 통행 시간뿐입니다. 조리 완료 시각과 주문별 약속 시간은 넣지
  않았습니다. 실제 배달 순서는 거리만이 아니라 지연 위험으로 정해집니다.
- 주소를 좌표로 바꾸는 지오코딩은 운영에서 여전히 카카오 API를 호출합니다. 외부 호출이 사라진
  것은 경로 계산 부분이고, 데모의 대체 변환은 실제 좌표가 아닙니다.
- 스냅 거리는 도보 속도 상수 하나로만 환산합니다. 건물 진입 동선과 층 이동은 넣지 않았습니다.
