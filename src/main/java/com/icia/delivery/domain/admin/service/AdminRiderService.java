...
public class AdminRiderService {
    ...
    public RiderDTO getRiderById(Long id) {
        return repository.findByRiderNoOptional(id)
                .map(RiderDTO::toDTO)
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "라이더 정보를 찾을 수 없습니다. riderNo=" + id));
    }

    @Transactional
    public boolean updateRiderInfo(Long riderNo, RiderDTO riderForm) {
        RiderEntity rider = repository.findByRiderNoOptional(riderNo)
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "라이더 정보를 찾을 수 없습니다. riderNo=" + riderNo));

        // 필수 필드 체크 (예: riderName이 null이면 예외 처리)
        if (riderForm.getRiderName() == null || riderForm.getRiderName().isEmpty()) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "라이더 이름은 필수입니다.");
        }
